...
#from functions import check_flux_direction_torch, mass_cartesian_torch, momentum_nonlinear_cartesian,compute_dt_cfl
...
def apply_closed_boundary(H, M, N, Z):
    """
    简化版：
      1) 任何水位 0 <= H_layer < threshold 都强制设为 0
      2) 然后对边界做 clamp 处理
      3) 最后对 M、N 边界做清零

    假设:
      - H_layer 表示“相对于参考面0”的自由面标高 (η)，而床面 Z<0。
      - 你只想把略大于 0 的水位钉到 0。
    """
    Nx_, Ny_ = Z.shape
    layer = 1
    ...
    # 阈值: 0 < H_layer < threshold => 0
    threshold = 0.05
    mask_small_positive = (H_layer >= 0) & (torch.abs(H_layer + Z)< threshold)
    H_layer = torch.where(mask_small_positive, torch.zeros_like(H_layer), H_layer)

    # 2) 做边界 clamp
    left_boundary = torch.clamp(
        Z[0, :] + H_layer[0, :],
        min=Z[0, :],
        max=torch.zeros_like(Z[0, :])
    ) - Z[0, :]
    right_boundary = torch.clamp(
        Z[-1, :] + H_layer[-1, :],
        min=Z[-1, :],
        max=torch.zeros_like(Z[-1, :])
    ) - Z[-1, :]
    bottom_boundary = torch.clamp(
        Z[:, 0] + H_layer[:, 0],
        min=Z[:, 0],
        max=torch.zeros_like(Z[:, 0])
    ) - Z[:, 0]
    top_boundary = torch.clamp(
        Z[:, -1] + H_layer[:, -1],
        min=Z[:, -1],
        max=torch.zeros_like(Z[:, -1])
    ) - Z[:, -1]
    ...
    H_layer_updated = torch.where(rows == 0,
                                  left_boundary.unsqueeze(0).expand(Nx, Ny),
                                  H_layer_updated)
    H_layer_updated = torch.where(rows == Nx - 1,
                                  right_boundary.unsqueeze(0).expand(Nx, Ny),
                                  H_layer_updated)
    H_layer_updated = torch.where(cols == 0,
                                  bottom_boundary.unsqueeze(1).expand(Nx, Ny),
                                  H_layer_updated)
    H_layer_updated = torch.where(cols == Ny - 1,
                                  top_boundary.unsqueeze(1).expand(Nx, Ny),
                                  H_layer_updated)
    ...
    if M_Nx > 1:
        M_layer_updated = torch.where(M_rows == 0,
                                      torch.zeros_like(M_layer_updated),
                                      M_layer_updated)
        M_layer_updated = torch.where(M_rows == M_Nx - 1,
                                      torch.zeros_like(M_layer_updated),
                                      M_layer_updated)
    ...
    if N_Ny > 1:
        N_layer_updated = torch.where(N_cols == 0,
                                      torch.zeros_like(N_layer_updated),
                                      N_layer_updated)
        N_layer_updated = torch.where(N_cols == N_Ny - 1,
                                      torch.zeros_like(N_layer_updated),
                                      N_layer_updated)
...
Fx_true = np.load("fx_pt_array_1.npy", allow_pickle=True)
Fy_true = np.load("fy_pt_array_1.npy", allow_pickle=True)
...
for epoch in range(num_epochs):
    ...
    for t in range(num_time_steps_train):
        ...
        #print("FX MAX =", torch.max(Fx_t))
        #print("FY MAX =", torch.max(Fy_t))
        ...
        # 物理更新
        H_new = mass_cartesian_torch(H, Z, M, N)
        D_M, D_N = reconstruct_flow_depth_torch(H_new, Z, M, N, dry_limit)
        #M_new, N_new, _, _ = momentum_nonlinear_cartesian(H_new, Z, M, N, D_M, D_N, dt_current, Fx_t, Fy_t)
        M_new, N_new, _, _ = momentum_nonlinear_cartesian(H_new, Z, M, N, D_M, D_N, dt_current, Fx_t, Fy_t)
        H_new, M_new, N_new = apply_closed_boundary(H_new, M_new, N_new, Z)
        
        print("eta_tensor[t] min =", eta_tensor[t].min().item())
        print("eta_tensor[t] max =", eta_tensor[t].max().item())
        
        plot(Fx_true[t].flatten(), dd(Fx_t[:-1].flatten()));
        #plot(Fy_true[t].flatten(), dd(F_true.flatten()),'r');
        show()
        ...
        if epoch == num_epochs - 1:
            eta_list.append(H_new[1].detach().cpu().numpy())
            u_list.append(M_new[1].detach().cpu().numpy())
            v_list.append(N_new[1].detach().cpu().numpy())
        ...
        if (t + 1) % chunk_size == 0 or (t == num_time_steps_train - 1):
            ...
            # 截断计算图，更新状态
            H = torch.stack((H_new[1].clone(), H_new[1].clone()), dim=0).detach().requires_grad_(True)
            M = torch.stack((M_new[1].clone(), M_new[1].clone()), dim=0).detach().requires_grad_(True)
            N = torch.stack((N_new[1].clone(), N_new[1].clone()), dim=0).detach().requires_grad_(True)
            acc_loss = 0.0

        else:  # 继续传播梯度，不 detach
            H = torch.stack((H_new[1].clone(), H_new[1].clone()), dim=0)
            M = torch.stack((M_new[1].clone(), M_new[1].clone()), dim=0)
            N = torch.stack((N_new[1].clone(), N_new[1].clone()), dim=0)
...
# 保存数据
np.save("eta_list.npy", np.array(eta_list))
np.save("u_list.npy", np.array(u_list))
np.save("v_list.npy", np.array(v_list))
np.save("loss_list.npy", np.array(train_loss_history))
