    ...
    for t in range(params.NT):
        ...
        if t>12:
            ...
            for i in range(inner_steps):
                ...
                #tic()
                # Fx = rho2u(manning**2)
                # M_new = H_work[0] + dt  * Fx
                
                # Fy = rho2v(manning**2)
                # N_new = N_work[1] + dt  * Fy
                manning = model(current_input).squeeze(0).squeeze(0)        
                #
                ...
                M_update, N_update, Fx, Fy = momentum_nonlinear_cartesian_torch(H_init, Z, M_init, N_init, Wx[0], Wy[0], Pa[0], params, manning)
                H_update = mass_cartesian_torch(H_init, Z, M_update, N_update, params)
