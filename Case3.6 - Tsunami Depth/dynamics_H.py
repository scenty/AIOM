...
def mass_cartesian_torch(H, Z, M, N, params):
    ...
    # —— 关键：构造一个 delta 张量，完全 out‑of‑place
    delta = torch.zeros_like(H0)
    # 只在内点更新
    delta[1:-1, 1:-1] = - CC1 * dMdx[1:-1, 1:-1]  \
                       - CC2 * dNdy[1:-1, 1:-1] 
    # 直接相加，得到 H1
    H1 = H0 + delta

    # —— 后续所有修正都用 torch.where，不做任何原地赋值
    # 干湿修正
...
def momentum_nonlinear_cartesian_torch(H, Z, M, N, Wx, Wy, Pa, params, Z_recorrexted):
    ...
    Cf = getattr(params, 'manning', 1e-2)
    dt = params.dt
    ...
    ududx = F.pad( ududx_up(M[0],N[0],Z+H[1]), (0,0,1,1)) # reorder
    vdudy = F.pad( vdudy_up(M[0],N[0],Z+H[1]), (1,1,0,0)) # reorder
    ...
    #Fx = g * Cf_u**2 / (D0**2.33 + 1e-9) * torch.sqrt(m0**2 + Nu**2 + eps) * m0
    #by LWF, when optimize, do not use clamping, replacing, or non-torch operations
    Fx = g * Cf_u**2 / (D0**2.33 + 1e-9) * torch.sqrt(torch.clamp(m0**2 + Nu**2, min=eps)) * m0
    ...
    safe_D0 = torch.clamp(D0, min=MinWaterDepth)
    phi_M = 1.0 - CC1 * torch.min(torch.abs(m0 / safe_D0),torch.sqrt(g * safe_D0 + 1e-12))
    M_new = M.clone() #by LWF    
    ...
    udvdx = F.pad( udvdx_up(M[0],N[0],Z+H[1]), (0,0,1,1)) # reorder
    vdvdy = F.pad( vdvdy_up(M[0],N[0],Z+H[1]), (1,1,0,0)) # reorder
    ...
    Cf_v = rho2v(Cf)  
    eps = 1e-6
    #Fy = g * Cf_v**2 / (D0N**2.33 + 1e-9) * torch.sqrt(Mv**2 + n0**2 + eps) * n0
    #by LWF, when optimize, do not use clamping, replacing, or non-torch operations
    Fy = g * Cf_v**2 / (D0N**2.33 + 1e-9) * torch.sqrt(torch.clamp(Mv**2 + n0**2, min=eps)) * n0
    ...
    N_new = N.clone() #by LWF
    safe_D0N = torch.clamp(D0N, min=MinWaterDepth)
    phi_N = 1.0 - CC2 * torch.min(torch.abs(n0 / safe_D0N),torch.sqrt(g * safe_D0N + 1e-12)) 
...
#0418 by LWF new bcond vectorized:
def bcond_zeta_vec(H, Z, params, tide_z=None):
    """
...
def bcond_u2D_vec(H, Z, M, D_M, flux_sign, z_w, z_e, params):
    ...
    M_new = torch.where(mask_u.to(torch.bool), torch.tensor(0.0, dtype=torch.float64, device=M_new.device), M_new)
...
def bcond_v2D_vec(H, Z, N, D_N, flux_sign, z_s, z_n, params):
    ...
    N_new = torch.where(mask_v.to(torch.bool), torch.tensor(0.0, dtype=torch.float64, device=N_new.device), N_new)
